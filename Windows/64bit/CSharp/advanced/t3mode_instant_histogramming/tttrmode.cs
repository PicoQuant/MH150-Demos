...
namespace tttrmode
{
  class tttrmode
  {
    ...
    static uint[,] histogram = new uint[Constants.MAXINPCHAN, T3HISTBINS];
    ...
    static void Main(string[] args)
    {
      ...
      int Mode = Constants.MODE_T3;  // Leave as is, this demo is only for T3 mode!
      ...
      try
      {
        fs = new FileStream("t3hist.out", FileMode.Create,  FileAccess.Write);
      ...
      retcode = mhlib.MH_SetSyncChannelOffset(dev[0], -10000);    //in ps, emulate a cable delay
      if (retcode < 0)
      {
        mhlib.MH_GetErrorString(Errstr, retcode);
      ...
      for (i = 0; i < T3HISTBINS; i++)
        for (j = 0; j < Constants.MAXINPCHAN; j++)
            histogram[j, i] = 0;
