...
inputChannelOffset = 50000 # you can change this (in ps, like a cable delay)
...
mainfilter_timerange = 250; # in picoseconds
mainfilter_matchcnt = 1      # must have at least one other event in proximity
mainfilter_inverse = 0       # normal filtering mode, see manual
mainfilter_enable = 1        # activate the filter with 1, deactivate with 0
...
for i in range(0,inputrows):
    tryfunc(mhlib.MH_SetMainEventFilterChannels(ct.c_int(dev[0]), ct.c_int(i), ct.c_int(mainfilter_usechans[i]), ct.c_int(mainfilter_passchans[i])),"SetMainEventFilterChannels")

tryfunc(mhlib.MH_SetMainEventFilterParams(ct.c_int(dev[0]), mainfilter_timerange, mainfilter_matchcnt, mainfilter_inverse),"SetMainEventFilterParam")
...
#tryfunc(mhlib.MH_GetWarnings(ct.c_int(dev[0]), warnings), "GetWarnings")  
#if warnings > 0:
#    tryfunc(mhlib.MH_GetWarningsText(ct.c_int(dev[0]), warningstext, warnings))
...
#for i in range(0, numChannels.value):     
#    print('ftestchanrates[',i,'] after GetAllCountRates: ',ftestchanrates[i]) 
...
if mode == MODE_T2:
    ftestsumrate = ftestsumrate + ftestsyncrate.value
print('\nMain Filter input rate = ', ftestsumrate, '/s')    
...
if mode == MODE_T2:
    ftestsumrate = ftestsumrate + ftestsyncrate.value
print('\nMain Filter output rate = ', ftestsumrate, '/s') 
